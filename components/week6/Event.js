...
export default function TourFlatList(props) {
    ...
    return (
        <View style={props.style}>
            ...
            <Text style={{ color: "grey" }}>What's the world That Could Happend</Text>
            <FlatList
                horizontal={true}
                // data={tours}
                data={onlineTours}
                renderItem={
                    ({ item, index }) => {
                        ...
                        return (
                            <View style={{ marginRight: 10, borderColor: 'black', borderWidth: 1, borderRadius: 10 }}>
                                <Image
                                    style={{
                                        width: 300,
                                        height: 150,
                                ...
                                <View style={{ flexDirection: 'row', padding: 10 }}>
                                    <View>
                                        ...
                                        <Text style={{ fontSize: 16, color: 'black', textAlign: 'center' }}>{item.date}</Text>
                                    </View>
                                    <View style={{ flex: 1, paddingLeft: 10 }}>
                                        <Text style={{ fontSize: 20, color: 'black', textAlign: 'left' }}>{item.title}</Text>
                                        <Text style={{ fontSize: 16, color: 'black', textAlign: 'left' }}>{item.datetime}</Text>
                                        <Text style={{ fontSize: 16, color: 'black', textAlign: 'left' }}>{item.place}</Text>
                                    </View>
                                </View>
                            </View>
                        );
